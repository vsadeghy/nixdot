{
  ...
  inherit (import ../maps.nix {inherit lib;}) nmap mkMap;
in {
  programs.nixvim = {
    plugins.lazy.plugins = with pkgs.vimPlugins; [
      ...
      nvim-spectre
      {
        pkg = refactoring-nvim;
        dependencies = [nvim-treesitter plenary-nvim];
        lazy = false;
        config = true;
      }
    ];
    keymaps =
      nmap {
        "<leader>ts" = ["<cmd>Spectre<cr>" "Open Spectre"];
      }
      ++ mkMap ["n" "x"] {
        "<leader>rr" = ["<cmd>lua require('refactoring').select_refactor()<cr>" "Refactor"];
      };
  };
}
