    ...
    plugins.lazy.plugins = with pkgs.vimPlugins; [
      {
        ...
        config.__raw = ''
          function()
            ...
            map("<C-j>", function() harpoon:list():select(1) end, "Select 1")
            map("<C-k>", function() harpoon:list():select(2) end, "Select 2")
            map("<C-l>", function() harpoon:list():select(3) end, "Select 3")
            map("<C-;>", function() harpoon:list():select(4) end, "Select 4")
